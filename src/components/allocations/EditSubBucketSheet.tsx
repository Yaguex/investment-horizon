...
interface EditSubBucketSheetProps {
  isOpen: boolean
  onClose: () => void
  allocation: Allocation
}

export function EditSubBucketSheet({ isOpen, onClose, allocation }: EditSubBucketSheetProps) {
  ...
  const onSubmit = async (values: AllocationTradeFormValues) => {
    console.log('Submitting sub-bucket update with values:', values)
    ...
    try {
      ...
      if (updateError) {
        console.error('Error updating sub-bucket:', updateError)
        toast({
          title: "Error",
          description: "Failed to update sub-bucket",
          variant: "destructive",
        })
        throw updateError
      }
      
      console.log('Sub-bucket updated successfully, now recalculating values...')
      ...
      toast({
        title: "Success",
        description: "Sub-bucket updated successfully",
      })
      onClose()
    } catch (error) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Edit Sub-bucket</SheetTitle>
        </SheetHeader>
